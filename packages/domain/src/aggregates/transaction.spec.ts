...
describe("Transaction Aggregate", () => {
  ...
  const DUMMY_ITEM_NAME = "Test Item";
  ...
  const DUMMY_OWNER_ID = "owner-id-123";
  ...
  describe("Factory: createReservation", () => {
    ...
    it("should create a valid reservation", () => {
      ...
      expect(transaction.itemName).toBe(DUMMY_ITEM_NAME);
  ...
  describe("Lifecycle Transitions", () => {
    ...
    it("should be idempotent when cancelling an already cancelled transaction (No Friction)", () => {
      ...
      const again = cancelled.cancel();
      expect(again).toBe(cancelled);
    });

    it("should return same instance if cancelling a transaction already cancelled by owner (No Friction)", () => {
      const cancelled = Transaction.reconstitute({
        id: VALID_TRANSACTION_ID,
        itemId: VALID_ITEM_ID,
        userId: VALID_USER_ID,
